...
checks={
 ...
 'workspace-loading-tab':'Workspace compact loading tab missing',
...
if 'jp-boot-loader' not in html or 'jp-boot-spinner' not in html:
    raise SystemExit('Online compact loading state missing')
...
grep -q "juan-online-v1.3.3.2-platform-update-v134" "$ROOT/online/sw.js" || { echo 'Online SW cache version stale'; exit 1; }
grep -q "juan-workspace-v1.3.3.2-sync-center-ux-v135" "$ROOT/workspace/sw.js" || { echo 'Workspace SW cache version stale'; exit 1; }
...
echo 'JUAN PROJECT Platform V1.3.3.2 old-table + compact-loader patch verification passed.'
