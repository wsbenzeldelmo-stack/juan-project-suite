...
(function(){
  ...
  var data=null,bannerIndex=0,timer=null,impressed=new Set();
  ...
  function mountBanners(){
    if(!data)return;var banners=(data.ads||[]).filter(function(a){return a.ad_type==="banner"&&a.placement==="homepage_banner"&&a.image;});if(!banners.length){document.getElementById("jpHouseBanner")?.remove();return;}
  ...
  function popup(){
    if(!data||sessionStorage.getItem(POPUP_SESSION_KEY))return;
    var ads=(data.ads||[]).filter(function(a){return a.ad_type==="popup"&&a.placement==="homepage_popup"&&a.image&&!localStorage.getItem("JUAN_AD_SEEN_"+a.id);}).sort(function(a,b){return Number(b.priority||0)-Number(a.priority||0);});
    var ad=ads[0];if(!ad)return;sessionStorage.setItem(POPUP_SESSION_KEY,"1");localStorage.setItem("JUAN_AD_SEEN_"+ad.id,"1");
  ...
  async function load(){try{data=await api({action:"ads"});render();}catch(e){console.warn("In-house ads unavailable:",e?.message||e);}}
  window.addEventListener("juan-online-render",function(){if(data)render();});
