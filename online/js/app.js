...
let state={
  ...
  notificationOpen:false,senderInstitution:'',guestGateContext:'default'
};
...
function payment(){
  ...
  return `${pageHead('Payment',{back:false,more:false})}${selected?paymentSummary(selected):''}<div class="tabs payment-tabs"><button class="active">Make a Payment</button><button id="paymentHistoryTab">History</button></div><div class="card payment-card"><div class="payment-card-head"><div><span class="eyebrow">RECEIVING ACCOUNT</span><b>UnionBank · JUAN PROJECT</b><p>Use the QR or account details below, then submit your payment proof.</p></div>${icon('payment',22)}</div><div class="qr-wrap"><img class="qr" src="${esc(qr)}" alt="UnionBank bank QR code"></div><div class="bank-meta"><div><span>Account Name</span><b>${esc(settings.account_name||'JUAN PROJECT')}</b></div><div><span>Account Number</span><b>${esc(settings.account_number||'•••• •••• 1710')}</b></div></div>${settings.instructions?`<p class="payment-instructions">${esc(settings.instructions)}</p>`:''}</div><div class="card payment-card secure-payment-card"><div class="secure-head"><div><span class="eyebrow">PAYMENT PROOF</span><b>Submit Payment Details</b><p>Every required field is checked before the payment can be sent for review.</p></div>${icon('receipt',22)}</div><div class="field institution-field"><label>Sending Bank / E-Wallet *</label><input id="payInstitutionSearch" class="input" list="paymentInstitutionOptions" autocomplete="off" placeholder="Type to search bank or e-wallet" value="${esc(institution?.name||'')}"><datalist id="paymentInstitutionOptions">${PAYMENT_INSTITUTIONS.map(x=>`<option value="${esc(x.name)}"></option>`).join('')}</datalist><div id="institutionValidation" class="field-error"></div></div><div id="institutionPreview" class="institution-preview ${institution?'':'hidden'}">${institution?`<img src="${esc(institution.logo)}" alt="${esc(institution.name)} logo"><div><b>${esc(institution.name)}</b><small>${esc(institution.type)} · ${esc(institution.hint)}</small></div>`:''}</div><label class="upload" for="receipt">${icon('upload',24)}<span id="receiptLabel">${state.receiptPath?'Receipt uploaded':'Choose receipt *'}</span><small>JPG, PNG, or PDF · max 5 MB</small><input id="receipt" type="file" accept="image/jpeg,image/png,application/pdf" hidden></label><div id="receiptValidation" class="field-error standalone-error"></div><div class="manual-payment-grid"><div class="field"><label>Amount Paid *</label><input id="payAmount" class="input" type="number" min="0.01" step="0.01" inputmode="decimal" placeholder="0.00"><div id="amountValidation" class="field-error"></div></div><div class="field"><label>Reference Number *</label><input id="payRef" class="input" autocomplete="off" placeholder="${esc(institution?.example||'Select sender first')}"><div id="referenceValidation" class="field-error"></div><div id="referenceHint" class="field-help">${institution?esc(institution.hint):'Reference format changes with the selected sender.'}</div></div><div class="field"><label>Payment Date *</label><input id="payDate" class="input" type="date"><div id="dateValidation" class="field-error"></div></div><div class="field"><label>Transfer Fee</label><input id="payTransferFee" class="input" type="number" min="0" step="0.01" inputmode="decimal" value="0"></div><div class="fee-readout net"><span>Net Amount</span><b id="payNetAmount">₱0.00</b></div></div><div class="field"><label>Apply to Project *</label><select id="payProjectSel" class="input">${(all||[]).map(p=>`<option value="${esc(p.id)}" ${selected?.id===p.id?'selected':''}>${esc(p.project_code||p.id)} · ${esc(p.title)}</option>`).join('')}</select></div><div id="paymentSystemCheck" class="payment-system-check"><b>System verification</b><span>Complete all required details before submitting.</span></div><button id="submitPayment" class="btn primary full" disabled>Submit Payment for Review</button><p class="form-help">Your balance changes only after JUAN PROJECT approves the payment.</p></div><div class="section-head payment-history"><h2>Payment History</h2></div><div class="card history-list">${(state.portal?.paymentSubmissions||[]).map(h=>`<div class="history-row"><div><b>${peso(h.submitted_amount)}</b><small>${esc(fmtDate(h.payment_date||h.submitted_at))} · ${esc(h.reference_number||'No reference')}</small>${h.status==='rejected'&&h.rejection_reason?`<small class="history-reason">Reason: ${esc(h.rejection_reason)}</small>`:''}</div><span class="badge ${String(h.status||'').toLowerCase()==='pending'?'pending':''}">${esc(h.status||'Pending')}</span></div>`).join('')||'<div class="empty compact-empty">No payment submissions yet.</div>'}</div>`;
}
...
function bind(){
  ...
  const resolveInstitution=()=>PAYMENT_INSTITUTIONS.find(x=>x.name.toLowerCase()===String(institutionInput?.value||'').trim().toLowerCase())||null;
  ...
  if(institutionInput){institutionInput.addEventListener('input',refreshInstitution);institutionInput.addEventListener('change',refreshInstitution)}
  ...
  if(receipt)receipt.onchange=async()=>{const f=receipt.files?.[0];if(!f)return;try{if(!['image/jpeg','image/png','application/pdf'].includes(f.type))throw Error('Receipt must be a JPG, PNG, or PDF file.');if(f.size>5*1024*1024)throw Error('Receipt must be 5 MB or smaller.');state.receiptPath=await uploadReceipt(f);if(receiptLabel)receiptLabel.textContent='Receipt uploaded';setInlineError('receiptValidation','');toast('Receipt uploaded securely.');validatePaymentForm()}catch(e){state.receiptPath=null;setInlineError('receiptValidation',e.message);toast(e.message);validatePaymentForm()}};
  ...
  if(submitBtn)submitBtn.onclick=async()=>{try{
    ...
    state.portal=await getPortal();state.receiptPath=null;state.senderInstitution='';
    root.innerHTML=`<div class="flow-screen success"><div class="flow-check">✓</div><span class="eyebrow">SUBMISSION RECEIVED</span><h2>Payment Submitted</h2><p>Your payment proof passed submission checks and is now waiting for JUAN PROJECT review.</p><div class="processing-steps payment-result-timeline"><span class="done">Details verified</span><span class="done">Receipt received</span><span class="done">Submitted successfully</span><span class="active-step">Pending review</span><span>Approval updates your balance</span></div><div class="success-summary"><div><span>Amount</span><b>${peso(amount)}</b></div><div><span>Status</span><b>Pending Review</b></div></div><button id="paymentBackOrder" class="btn primary full">Back to Order</button><button id="paymentHistoryDone" class="btn full">View Payment History</button></div>`;
