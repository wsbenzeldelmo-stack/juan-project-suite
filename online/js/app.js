...
const ONBOARDING_KEY='JUAN_ONBOARDING_DONE_V4';
...
let state={
  ...
  shopQuery:'',shopSort:'default',manualPaymentAllowed:false,paymentEntrySource:'gemini',paymentFlow:'',
...
const onboardingSlides=[
  {icon:'orders',title:'Everything about your project, in one place.',body:'Follow your project from confirmation to delivery with a clear order tracker.'},
  {icon:'check',title:'Know exactly what happens next.',body:'See deliverables, deadlines, and the next action without digging through messages.'},
  {icon:'folderlock',title:'Pay, track, and receive your files.',body:'Submit payments securely and unlock your project folder when delivery is ready.'}
];
function onboardingScreen(){
  const slide=onboardingSlides[state.onboardingStep];
  root.innerHTML=`<div class="onboard-shell"><div class="phone-page onboard-card"><button id="skipOnboard" class="text-button top-right">Skip</button><div class="onboard-main"><div class="onboard-icon">${icon(slide.icon,38)}</div><span class="eyebrow">${state.onboardingStep+1} OF ${onboardingSlides.length}</span><h2>${esc(slide.title)}</h2><p>${esc(slide.body)}</p><div class="dots">${onboardingSlides.map((_,i)=>`<span class="${i===state.onboardingStep?'active':''}"></span>`).join('')}</div></div><div class="onboard-actions"><button id="onboardBack" class="btn" ${state.onboardingStep===0?'disabled':''}>Back</button><button id="onboardNext" class="btn primary">${state.onboardingStep===onboardingSlides.length-1?'Explore JUAN PROJECT':'Continue'}</button></div></div></div>`;
...
function payment(){
  ...
  const manual=state.manualPaymentAllowed&&state.paymentEntrySource==='manual',x=state.extractedReceipt||{};
  const amount=x.amount??x.amount_paid??'',method=x.payment_method??x.method??'',ref=x.reference_number??x.reference??'',date=x.payment_date??x.date??'',fee=Number(x.transfer_fee??x.fee??0)||0,net=amount!==''?Math.max(0,Number(amount||0)-fee):'';
  return `${pageHead('Payment',{back:false,more:false})}${selected?paymentSummary(selected):''}<div class="tabs payment-tabs"><button class="active">Make a Payment</button><button id="paymentHistoryTab">History</button></div><div class="card payment-card"><div class="payment-card-head"><div><span class="eyebrow">BANK TRANSFER</span><b>Scan to Pay</b><p>Send your payment via ${esc(settings.method_label||'UnionBank InstaPay')}.</p></div>${icon('payment',22)}</div><div class="qr-wrap"><img class="qr" src="${esc(qr)}" alt="UnionBank bank QR code"></div><div class="bank-meta"><div><span>Account Name</span><b>${esc(settings.account_name||'JUAN PROJECT')}</b></div><div><span>Account Number</span><b>${esc(settings.account_number||'•••• •••• 1710')}</b></div></div>${settings.instructions?`<p class="payment-instructions">${esc(settings.instructions)}</p>`:''}</div><div class="card payment-card secure-payment-card"><div class="secure-head"><div><span class="eyebrow">SECURE RECEIPT CHECK</span><b>Upload Payment Receipt</b><p>JPG, JPEG, PNG, or PDF · maximum 5 MB</p></div><span class="secure-chip">Gemini Assisted</span></div><label class="upload" for="receipt">${icon('upload',24)}<span id="receiptLabel">${state.receiptPath?'Receipt uploaded':'Choose receipt'}</span><small>Tap to select a payment proof</small><input id="receipt" type="file" accept="image/jpeg,image/png,application/pdf" hidden></label><button id="readReceipt" class="btn full" ${!state.receiptPath?'disabled':''}>Verify Receipt with Gemini</button>${state.extractedReceipt||manual?`<div class="verification-banner ${manual?'manual':'verified'}">${icon(manual?'alert':'check',16)}<span>${manual?'Automatic verification is temporarily unavailable. Manual entry is enabled for this submission.':'Receipt details verified. These fields are locked for security.'}</span></div><div class="verified-fields">${verifiedField('Amount Paid','payAmount',amount,'number',!manual)}${verifiedField('Mode of Payment','payMethod',method,'text',!manual)}${verifiedField('Reference Number','payRef',ref,'text',!manual)}${verifiedField('Payment Date','payDate',date,'date',!manual)}<div class="fee-readout"><span>Transfer Fee</span><b>${peso(fee)}</b></div><div class="fee-readout net"><span>Net Amount</span><b>${net===''?'—':peso(net)}</b></div></div>`:`<div class="verification-placeholder"><span>${icon('spark',18)}</span><p>After upload, Gemini will read the transaction details and lock the verified fields.</p></div>`}<div class="field"><label>Apply to Project</label><select id="payProjectSel" class="input">${(all||[]).map(p=>`<option value="${esc(p.id)}" ${selected?.id===p.id?'selected':''}>${esc(p.project_code||p.id)} · ${esc(p.title)}</option>`).join('')}</select></div><button id="submitPayment" class="btn primary full" ${!(state.extractedReceipt||manual)?'disabled':''}>Submit Payment for Review</button></div><div class="section-head payment-history"><h2>Payment History</h2></div><div class="card history-list">${(state.portal?.paymentSubmissions||[]).map(h=>`<div class="history-row"><div><b>${peso(h.submitted_amount)}</b><small>${esc(fmtDate(h.payment_date||h.submitted_at))} · ${esc(h.reference_number||h.extracted_reference||'No reference')}</small></div><span class="badge ${String(h.status||'').toLowerCase()==='pending'?'pending':''}">${esc(h.status||'Pending')}</span></div>`).join('')||'<div class="empty compact-empty">No payment submissions yet.</div>'}</div>`;
}
...
function bind(){
  ...
  const projectSel=document.getElementById('payProjectSel');if(projectSel)projectSel.onchange=()=>{state.paymentProjectId=projectSel.value;state.extractedReceipt=null;render()};
  ...
  if(receipt)receipt.onchange=async()=>{const f=receipt.files?.[0];if(!f)return;try{if(f.size>5*1024*1024)throw Error('Receipt must be 5 MB or smaller.');state.extractedReceipt=null;state.manualPaymentAllowed=false;state.paymentEntrySource='gemini';state.receiptPath=await uploadReceipt(f);if(receiptLabel)receiptLabel.textContent='Receipt uploaded';if(readBtn)readBtn.disabled=false;toast('Receipt uploaded securely.')}catch(e){toast(e.message)}};
  if(readBtn)readBtn.onclick=async()=>{try{readBtn.disabled=true;readBtn.innerHTML=`<span class="btn-spinner"></span> Verifying…`;const x=await extractReceipt(state.receiptPath);state.extractedReceipt=x;state.manualPaymentAllowed=false;state.paymentEntrySource='gemini';toast('Payment details verified and locked.');render()}catch(e){if(e.status===429||e.status===501||e.code==='GEMINI_LIMIT'){state.manualPaymentAllowed=true;state.paymentEntrySource='manual';state.extractedReceipt={amount:'',payment_method:'',reference_number:'',payment_date:'',transfer_fee:0};toast('Gemini is temporarily unavailable. Manual entry is enabled for this submission.');render()}else toast(e.message||'Receipt verification failed.')}finally{if(document.body.contains(readBtn)){readBtn.disabled=false;readBtn.textContent='Verify Receipt with Gemini'}}};
  const submitBtn=document.getElementById('submitPayment');if(submitBtn)submitBtn.onclick=async()=>{try{if(!state.receiptPath)throw Error('Upload a receipt first.');const amt=document.getElementById('payAmount'),projectSel=document.getElementById('payProjectSel'),method=document.getElementById('payMethod'),ref=document.getElementById('payRef'),date=document.getElementById('payDate'),amount=Number(amt?.value||0);if(!(amount>0))throw Error('A verified amount is required.');if(!projectSel?.value)throw Error('Select a project.');submitBtn.disabled=true;root.innerHTML=`<div class="flow-screen"><div class="flow-spinner"></div><span class="eyebrow">SECURE PAYMENT</span><h2>Checking your payment…</h2><div class="processing-steps"><span class="done">Reading receipt</span><span>Matching transaction</span><span>Preparing submission</span></div><p>Please keep this page open for a moment.</p></div>`;await new Promise(r=>setTimeout(r,500));await submitPaymentApi({projectId:projectSel.value,amount,paymentMethod:method?.value||'',referenceNumber:ref?.value||'',paymentDate:date?.value||'',receiptPath:state.receiptPath,extracted:state.extractedReceipt,entrySource:state.paymentEntrySource});state.portal=await getPortal();state.receiptPath=null;state.extractedReceipt=null;state.manualPaymentAllowed=false;state.paymentEntrySource='gemini';root.innerHTML=`<div class="flow-screen success"><div class="flow-check">✓</div><span class="eyebrow">SUBMISSION RECEIVED</span><h2>Payment Submitted</h2><p>Your receipt is now pending JUAN PROJECT review. Your balance will update after approval.</p><div class="success-summary"><div><span>Amount</span><b>${peso(amount)}</b></div><div><span>Status</span><b>Pending Review</b></div></div><button id="paymentBackOrder" class="btn primary full">Back to Order</button><button id="paymentHistoryDone" class="btn full">View Payment History</button></div>`;document.getElementById('paymentBackOrder').onclick=()=>{state.selected=projectSel.value;state.route='project';render()};document.getElementById('paymentHistoryDone').onclick=()=>{state.route='payment';render();setTimeout(()=>document.querySelector('.payment-history')?.scrollIntoView({behavior:'smooth'}),20)}}catch(e){toast(e.message||'Payment could not be submitted.');state.route='payment';render()}};
  document.querySelectorAll('[data-toggle-pass]').forEach(b=>b.onclick=()=>{const input=document.getElementById(b.dataset.togglePass);if(input)input.type=input.type==='password'?'text':'password'});
