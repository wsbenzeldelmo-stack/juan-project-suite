...
function shopArtwork(x,large=false){
  const code=String(x?.product_code||'JP').toUpperCase(),kind=x?.kind==='Package'?'PACKAGE':categoryName(x?.category_id).toUpperCase();
  return `<div class="shop-art ${large?'large':''}"><div class="shop-art-top"><span>JUAN PROJECT</span><b>${esc(kind)}</b></div><div class="shop-art-mark"><img src="/assets/brand/j-mark.svg" alt=""><span>${esc(code)}</span></div><div class="shop-art-line"></div></div>`
}
function packageInclusions(x){if(x?.kind!=='Package')return[];return (state.catalog.packageItems||[]).filter(i=>String(i.package_id)===String(x.id)).sort((a,b)=>Number(a.sort_order||0)-Number(b.sort_order||0))}
...
function categoryName(id){return state.catalog.categories.find(c=>String(c.id)===String(id))?.name||'Service'}
function shopPrice(x){return x.kind==='Package'?Number(x.new_price||0):Number(x.price||0)}
function shopOldPrice(x){return x.kind==='Package'?Number(x.original_price||0):0}
function shopCard(x){
  const price=shopPrice(x),old=shopOldPrice(x),key=`${x.kind}:${x.id}`,incs=packageInclusions(x),isValue=old>price;
  return `<button class="shop-card-v2" data-view-shop="${esc(key)}"><div class="shop-card-media">${shopArtwork(x)}${isValue?'<span class="shop-card-badge">Best value</span>':''}</div><div class="shop-card-content"><div class="shop-card-topline"><span class="shop-kind-pill">${esc(x.kind==='Package'?'Package':categoryName(x.category_id))}</span><span class="shop-code">${esc(x.product_code||'')}</span></div><h3>${esc(x.name)}</h3><p>${esc(x.description||'JUAN PROJECT creative service.')}</p>${incs.length?`<div class="shop-card-inclusions">${incs.slice(0,3).map(i=>`<span>${icon('check',11)}${esc(i.item_name||'Included')}</span>`).join('')}${incs.length>3?`<span class="more">+${incs.length-3} more</span>`:''}</div>`:''}<div class="shop-card-footer"><div class="shop-card-price">${isValue?`<span>${peso(old)}</span>`:''}<b>${peso(price)}</b></div><span class="shop-view-link">View details ${icon('chevron',14)}</span></div></div></button>`
}
function shop(){
  const all=[...(state.catalog.services||[]).map(x=>({...x,kind:'Service'})),...(state.catalog.packages||[]).map(x=>({...x,kind:'Package'}))];
  const q=String(state.shopQuery||'').trim().toLowerCase(),pref=state.shopSort||'default',cat=String(state.shopCategory||'all');
  let items=all.filter(x=>!q||[x.product_code,x.name,x.description,categoryName(x.category_id),x.kind].some(v=>String(v||'').toLowerCase().includes(q)));
  if(cat==='packages')items=items.filter(x=>x.kind==='Package');else if(cat!=='all')items=items.filter(x=>String(x.category_id)===cat);
  const code=x=>Number(String(x.product_code||'').match(/(\d+)$/)?.[1]||999999);items.sort(pref==='price-desc'?(a,b)=>shopPrice(b)-shopPrice(a)||code(a)-code(b):pref==='price-asc'?(a,b)=>shopPrice(a)-shopPrice(b)||code(a)-code(b):(a,b)=>code(a)-code(b));
  const categories=[{id:'all',name:'All'},{id:'packages',name:'Packages'},...(state.catalog.categories||[])];
  const featured=(state.catalog.packages||[]).map(x=>({...x,kind:'Package'})).sort((a,b)=>shopPrice(b)-shopPrice(a))[0]||all[0]||null;
  const featuredKey=featured?`${featured.kind}:${featured.id}`:'';
  return `${pageHead('Shop',{back:false,more:false})}<section class="shop-hero-v2"><div class="shop-hero-copy"><span class="shop-hero-eyebrow">CREATIVE SERVICES · MADE SIMPLE</span><h2>Your idea.<br><strong>Our production.</strong></h2><p>Broadcast graphics, multimedia, tutorials, and production services built around your project.</p><div class="shop-hero-trust"><span>${icon('check',13)} Clear pricing</span><span>${icon('clock',13)} 14-day standard</span></div></div><div class="shop-hero-brand"><img src="/assets/brand/j-mark.svg" alt="JUAN PROJECT"></div></section>${featured?`<section class="shop-featured"><div class="section-head inner"><h2>Featured</h2><span>Recommended package</span></div><button class="shop-featured-card" data-view-shop="${esc(featuredKey)}">${shopArtwork(featured,true)}<div class="shop-featured-copy"><div><span class="shop-kind-pill">${esc(featured.kind==='Package'?'Package':categoryName(featured.category_id))}</span><h3>${esc(featured.name)}</h3><p>${esc(featured.description||'Complete JUAN PROJECT creative package.')}</p></div><div class="shop-featured-bottom"><b>${peso(shopPrice(featured))}</b><span>View package ${icon('chevron',14)}</span></div></div></button></section>`:''}<div class="shop-tools-v2"><label class="shop-search-v2">${icon('help',17)}<input id="shopSearch" value="${esc(state.shopQuery)}" placeholder="Search services or packages"></label><select id="shopSort" class="shop-sort-v2" aria-label="Sort services"><option value="default" ${pref==='default'?'selected':''}>Recommended</option><option value="price-asc" ${pref==='price-asc'?'selected':''}>Price ↑</option><option value="price-desc" ${pref==='price-desc'?'selected':''}>Price ↓</option></select></div><div class="shop-filter-scroll">${categories.map(c=>`<button class="shop-filter-chip ${cat===String(c.id)?'active':''}" data-shop-filter="${esc(String(c.id))}">${esc(c.name)}</button>`).join('')}</div><div class="shop-results-head"><b>${items.length} ${items.length===1?'service':'services'}</b><span>${q?'Search results':'Browse the catalog'}</span></div><div class="shop-grid-v2">${items.map(shopCard).join('')||'<div class="card empty guided-empty shop-empty-v2"><b>No matching services</b><span>Try another keyword or category.</span></div>'}</div><div class="shop-assurance"><div class="shop-assurance-icon">${icon('spark',18)}</div><div><b>Built around your project</b><span>Browse freely. Log in only when you are ready to request a service.</span></div></div>`;
}
...
function render(){root.innerHTML=`<div class="app"><main class="page">${routePage()}</main>${nav()}${gateOverlay()}${shopOverlay()}${notificationOverlay()}${clientMessageOverlay()}</div>`;bind()}
...
function bind(){
  ...
  const refreshInstitution=()=>{
    const inst=resolveInstitution();state.senderInstitution=inst?.code||'';
  ...
  const updateNet=()=>{const a=Number(document.getElementById('payAmount')?.value||0),f=Math.max(0,Number(document.getElementById('payTransferFee')?.value||0)),n=Math.max(0,a-f),el=document.getElementById('payNetAmount');if(el)el.textContent=peso(n);validatePaymentForm()};
