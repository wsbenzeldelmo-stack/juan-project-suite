...
let state={
  ...
  notificationOpen:false,senderInstitution:'',guestGateContext:'default',receiptPreviewUrl:'',receiptPreviewType:'',receiptPreviewName:''
};
...
function activityFeed(){const rows=[];(state.portal?.paymentSubmissions||[]).slice(0,4).forEach(x=>rows.push({date:new Date(x.submitted_at||x.payment_date||0),title:'Payment submitted',sub:`${peso(x.submitted_amount)} · ${paymentStatusLabel(x.status)}`,icon:'payment'}));for(const p of state.portal?.projects||[]){for(const d of p.deliverables||[]){if(d.completed)rows.push({date:new Date(d.completed_at||d.due_date||p.deadline_date||0),title:'Deliverable completed',sub:`${p.project_code||''} · ${d.item_name||'Deliverable'}`,icon:'check'})}}return rows.filter(x=>!Number.isNaN(x.date.getTime())).sort((a,b)=>b.date-a.date).slice(0,5)}
...
function invoice(){
  ...
  const explicitKnown=Math.max(0,Number(p.rush_fee||0))+Math.max(0,Number(p.workload_surcharge||0))+Math.max(0,Number(p.system_maintenance_fee||0)),subtotal=Math.max(0,Number(p.subtotal_amount||itemSubtotal||(storedTotal-explicitKnown+discount)));
