...
(function(){
  ...
  function openCart(){
    var items=cart();
    var trashIcon='<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M4 7h16"/><path d="M9 7V4h6v3"/><path d="m7 7 1 13h8l1-13"/><path d="M10 11v5M14 11v5"/></svg>';
    var cartIcon='<svg viewBox="0 0 24 24" width="36" height="36" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="9" cy="20" r="1"/><circle cx="18" cy="20" r="1"/><path d="M3 4h2l2.2 10.2a2 2 0 0 0 2 1.6h7.9a2 2 0 0 0 2-1.6L20.5 8H6"/></svg>';
    var html='<div class="jp-flow-title"><h1>Your Cart</h1><button id="jpClearCart" class="jp-icon-clear" aria-label="Clear cart">'+trashIcon+'</button></div>';
    if(!items.length)html+='<div class="jp-flow-empty"><div class="jp-empty-cart-icon">'+cartIcon+'</div><h2>Your cart is empty.</h2><p>Browse JUAN PROJECT services and add something when you are ready.</p><button id="jpBrowse" class="jp-mobile-primary">Browse Services</button></div>';
    else html+='<div class="jp-cart-list">'+items.map(function(i,n){return '<article class="jp-cart-item">'+thumb(i)+'<div class="jp-cart-copy"><h3>'+esc(i.name)+'</h3><b>'+peso(i.price)+'</b><div class="jp-qty"><button data-minus="'+n+'" aria-label="Decrease quantity">−</button><span>'+Number(i.qty||1)+'</span><button data-plus="'+n+'" aria-label="Increase quantity">+</button></div></div><button class="jp-cart-remove" data-remove="'+n+'" aria-label="Remove item">'+trashIcon+'</button></article>';}).join("")+'</div><div class="jp-promo-row jp-promo-info"><div><b>Promotions</b><small>Eligible JUAN PROJECT promos are applied during review.</small></div></div><div class="jp-cart-summary"><div><span>Subtotal</span><b>'+peso(total(items))+'</b></div><div class="total"><span>Estimated Total</span><strong>'+peso(total(items))+'</strong></div></div><button id="jpCheckout" class="jp-mobile-primary">Continue to Checkout →</button>';
