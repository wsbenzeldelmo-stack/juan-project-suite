...
(function(){
  ...
  async function renderAds(){
    var host=ensureAdsPage();if(!host)return;host.innerHTML='<div class="jp-ad-loading"><span class="portal-loading-spinner"></span><div><strong>Loading In-House Ads</strong><small>Syncing campaigns and analytics…</small></div></div>';
    try{
      var d=await API({action:"ad-dashboard"}),ads=d.ads||[],stats=d.stats||{},settings=d.settings||{rotation_seconds:8},nonArchived=ads.filter(function(a){return adStatus(a)!=="Archived";});
      var active=nonArchived.filter(function(a){return adStatus(a)==="Live";}).length,banners=nonArchived.filter(function(a){return a.ad_type==="banner";}).length,popups=nonArchived.filter(function(a){return a.ad_type==="popup";}).length,scheduled=nonArchived.filter(function(a){return adStatus(a)==="Scheduled";}).length;
      host.innerHTML='<div class="jp-ads-page"><div class="jp-ads-heading"><div><span class="section-kicker">PROMOTIONS</span><h1>In-House Ads</h1><p>Create, publish, and monitor banners and popups displayed in JUAN PROJECT Online.</p></div><div class="jp-ads-heading-actions"><button class="btn btn-secondary" id="jpBannerSettings">Banner Settings</button><button class="btn btn-primary" id="jpCreateAd">+ Create Ad</button></div></div>'+
        '<div class="jp-ad-kpis"><div><span>Live</span><strong>'+active+'</strong><small>currently visible</small></div><div><span>Banners</span><strong>'+banners+'</strong><small>active records</small></div><div><span>Popups</span><strong>'+popups+'</strong><small>active records</small></div><div><span>Scheduled</span><strong>'+scheduled+'</strong><small>upcoming campaigns</small></div></div>'+
        '<section class="jp-ad-management"><div class="jp-ad-toolbar"><div class="jp-ad-search-wrap"><span aria-hidden="true">⌕</span><input id="jpAdSearch" class="form-control" placeholder="Search campaign or destination…"></div><div class="filter-pills" id="jpAdFilters"><button class="filter-pill active">All</button><button class="filter-pill">Banners</button><button class="filter-pill">Popups</button><button class="filter-pill">Scheduled</button><button class="filter-pill">Archived</button></div></div>'+
        '<div class="jp-ad-table-wrap"><table class="data-table unified-table jp-ad-table"><thead><tr><th>Campaign</th><th>Type</th><th>Status</th><th>Placement</th><th>Audience</th><th>Schedule</th><th>Performance</th><th class="table-actions-col"></th></tr></thead><tbody id="jpAdRows"></tbody></table></div></section>'+
        '<div class="jp-ad-rules"><div><strong>Display rules</strong><span>Popups show once per campaign per viewer. Banners rotate without being dismissible.</span></div><small>Rotation: '+Number(settings.rotation_seconds||8)+' seconds · Fade transition</small></div></div>';
      ...
      function draw(){
        var q=(search.value||"").toLowerCase().trim(),list=ads.filter(function(a){var s=adStatus(a);if(current==="All"&&s==="Archived")return false;if(current==="Banners"&&(a.ad_type!=="banner"||s==="Archived"))return false;if(current==="Popups"&&(a.ad_type!=="popup"||s==="Archived"))return false;if(current==="Scheduled"&&s!=="Scheduled")return false;if(current==="Archived"&&s!=="Archived")return false;return !q||((a.title||"")+" "+adPlacementLabel(a.placement)+" "+adAudienceLabel(a.audience)+" "+adDestinationLabel(a)).toLowerCase().indexOf(q)>=0;});
        rows.innerHTML=list.map(function(a){var st=stats[a.id]||{},imp=Number(st.impression||0),clk=Number(st.click||0),ctr=imp?((clk/imp)*100).toFixed(1):"0.0",status=adStatus(a),menu="adMenu_"+String(a.id).replace(/-/g,""),statusClass=status==="Live"?"badge-green":status==="Expired"?"badge-red":"badge-neutral",schedule=(a.start_at?esc(dateTime(a.start_at)):"Now")+'<span class="jp-secondary-cell">'+(a.no_expiration?"No expiration":a.end_at?"Ends "+esc(dateTime(a.end_at)):"No end date")+"</span>",archiveAction=status==="Archived"?"":'<button class="popover-action" data-aarchive="'+a.id+'">Archive</button>';return '<tr><td><strong>'+esc(a.title||"Untitled campaign")+'</strong><span class="jp-secondary-cell">'+esc(adDestinationLabel(a))+'</span></td><td><span class="badge badge-neutral">'+esc(a.ad_type==="popup"?"Popup":"Banner")+'</span></td><td><span class="badge '+statusClass+'">'+esc(status)+'</span></td><td><strong class="jp-ad-cell-primary">'+esc(adPlacementLabel(a.placement))+'</strong></td><td>'+esc(adAudienceLabel(a.audience))+'</td><td>'+schedule+'</td><td><strong>'+imp.toLocaleString()+' views</strong><span class="jp-secondary-cell">'+clk.toLocaleString()+' clicks · '+ctr+'% CTR</span></td><td class="table-row-actions"><div class="popover-wrap" id="'+menu+'"><button class="icon-more-button vertical-more" aria-label="Campaign actions" onclick="app.togglePopover(\''+menu+'\',event)">⋮</button><div class="popover-panel client-row-menu"><button class="popover-action" data-aedit="'+a.id+'">Edit</button><button class="popover-action" data-atoggle="'+a.id+'">'+(status==="Live"?"Pause":"Publish")+'</button>'+archiveAction+'<button class="popover-action text-danger" data-adelete="'+a.id+'">Delete permanently</button></div></div></td></tr>';}).join("")||'<tr><td colspan="8" class="text-center text-muted py-4">No matching campaigns.</td></tr>';
        ...
        rows.querySelectorAll("[data-atoggle]").forEach(function(b){b.onclick=async function(){var a=ads.find(function(x){return x.id===b.dataset.atoggle;}),wasLive=adStatus(a)==="Live";try{await API({action:"save-ad",ad:Object.assign({},a,{status:wasLive?"paused":"published"})});toast(wasLive?"Campaign paused.":"Campaign published.");renderAds();}catch(e){toast(e.message);}};});
        rows.querySelectorAll("[data-aarchive]").forEach(function(b){b.onclick=async function(){var a=ads.find(function(x){return x.id===b.dataset.aarchive;});if(!(await confirmAction({title:"Archive "+(a?.title||"campaign")+"?",message:"The campaign will stop appearing in JUAN PROJECT Online but its analytics will remain available.",confirmLabel:"Archive",danger:true})))return;try{await API({action:"archive-ad",id:b.dataset.aarchive});toast("Campaign archived.");renderAds();}catch(e){toast(e.message);}};});
        rows.querySelectorAll("[data-adelete]").forEach(function(b){b.onclick=async function(){var a=ads.find(function(x){return x.id===b.dataset.adelete;});if(!(await confirmAction({title:"Delete "+(a?.title||"campaign")+" permanently?",message:"This permanently removes the campaign and its ad analytics. This cannot be undone.",confirmLabel:"Delete permanently",danger:true})))return;try{var result=await API({action:"delete-ad",id:b.dataset.adelete});toast(result.image_cleanup_warning?"Campaign deleted. The old image could not be removed from storage.":"Campaign permanently deleted.");renderAds();}catch(e){toast(e.message);}};});
      }
  ...
  function openAd(a,done){
    ...
    var m=modal('<header class="jp-general-modal-head"><div><span class="section-kicker">'+(a.id?"EDIT CAMPAIGN":"CREATE AD")+'</span><h2>'+(a.id?esc(a.title):"New In-House Ad")+'</h2></div><button class="jp-general-x">×</button></header><div class="jp-ad-editor-grid">'+
      ...
      '<div class="form-group"><label class="form-label">Promotional Image</label><input id="adFile" class="form-control" type="file" accept="image/jpeg,image/png,image/webp"><small class="text-muted">Banner format: 1800 × 600 px (3:1). Banner uploads are cropped to this size automatically.</small></div><div class="form-group"><label class="form-label">Destination</label><select id="adDestType" class="form-control"><option value="no_action">No Action</option><option value="shop">Shop</option><option value="package">Package</option><option value="service">Service</option><option value="referral">Referral</option><option value="loyalty">Loyalty</option><option value="page">Specific JPO Page</option><option value="external_url">External URL</option></select></div>'+
      '<div class="form-group"><label class="form-label">Destination Value</label><input id="adDest" class="form-control" value="'+esc(a.destination_value||"")+'"></div><div class="form-group"><label class="form-label">Audience</label><select id="adAudience" class="form-control"><option value="all">Everyone</option><option value="guest">Guests only</option><option value="client">Signed-in clients only</option></select></div><div class="form-group"><label class="form-label">Online Placement</label><select id="adPlacement" class="form-control"><optgroup label="Banner placements"><option value="guest_home_banner">Guest Home</option><option value="guest_shop_banner">Guest Shop</option><option value="client_home_banner">Client Portal Home</option><option value="client_shop_banner">Client Portal Shop</option><option value="homepage_banner">Legacy Home Banner</option></optgroup><optgroup label="Popup placements"><option value="guest_home_popup">Guest Home Popup</option><option value="client_home_popup">Client Portal Home Popup</option><option value="homepage_popup">Legacy Home Popup</option></optgroup></select></div>'+
    ...
    m.querySelector(".jp-general-x").onclick=closeOverlay;document.getElementById("adType").value=a.ad_type||"banner";document.getElementById("adDestType").value=a.destination_type||"no_action";document.getElementById("adAudience").value=a.audience||"all";document.getElementById("adPlacement").value=a.placement||((a.ad_type||"banner")==="popup"?"homepage_popup":"homepage_banner");document.getElementById("adNoExpiry").checked=!!a.no_expiration;
    function syncAdPlacement(){var type=document.getElementById("adType").value,placement=document.getElementById("adPlacement"),isPopup=type==="popup";Array.from(placement.options).forEach(function(o){o.disabled=isPopup?!o.value.endsWith("_popup"):o.value.endsWith("_popup");});if(placement.selectedOptions[0]?.disabled)placement.value=isPopup?"guest_home_popup":"guest_home_banner";}
    document.getElementById("adType").onchange=syncAdPlacement;syncAdPlacement();
    ...
    async function save(status){
      ...
      try{if(file)image=await uploadAd(file,v("adType"));var v=function(id){return document.getElementById(id).value;};await API({action:"save-ad",ad:Object.assign({},a,{id:a.id||null,title:title,ad_type:v("adType"),status:status,image:image,placement:v("adPlacement"),destination_type:v("adDestType"),destination_value:v("adDest").trim(),start_at:v("adStart")?new Date(v("adStart")).toISOString():null,end_at:v("adEnd")?new Date(v("adEnd")).toISOString():null,no_expiration:document.getElementById("adNoExpiry").checked,priority:Number(v("adPriority")||0),image_alt:v("adAlt").trim()||title,audience:v("adAudience")})});toast(status==="published"?"Campaign published.":"Draft saved.");closeOverlay();done();}catch(e){toast(e.message);}
    }
